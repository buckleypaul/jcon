...
static int test_putc(char c) {
    if (g_fail_next) return -1;
    if (g_buf_len + 1 >= BUF_CAP) return -1;
    g_buf[g_buf_len++] = c;
    return 0;
}
...
static const char *buf_cstr(void) {
    g_buf[g_buf_len] = '\0';
    return g_buf;
}
...
#define RUN(fn) do { g_current_test = #fn; buf_reset(); fn(); } while (0)
...
static void test_empty_object(void) {
    jcon_start(true, test_putc);
    EXPECT_EQ(jcon_end(), JCON_OK);
    EXPECT_STR_EQ(buf_cstr(), "{}");
}

static void test_empty_object_pretty(void) {
    jcon_start(false, test_putc);
    EXPECT_EQ(jcon_end(), JCON_OK);
    EXPECT_STR_EQ(buf_cstr(), "{}\n");
}
...
static void test_basic_minified(void) {
    ...
    jcon_add_int("b", 2);
    EXPECT_EQ(jcon_end(), JCON_OK);
    EXPECT_STR_EQ(buf_cstr(), "{\"a\":1,\"b\":2}");
}
...
static void test_array_of_ints(void) {
    ...
    jcon_array_end();
    EXPECT_EQ(jcon_end(), JCON_OK);
    EXPECT_STR_EQ(buf_cstr(), "{\"xs\":[1,2,3]}");
}
...
static void test_array_of_mixed(void) {
    ...
    jcon_array_end();
    EXPECT_EQ(jcon_end(), JCON_OK);
    EXPECT_STR_EQ(buf_cstr(), "{\"mix\":[1,\"two\",true,null]}");
}
...
static void test_io_error_sticky(void) {
    g_fail_next = true;
    jcon_start(true, test_putc);          /* first putc fails -> JCON_ERR_IO */
...
static void test_start_clears_sticky(void) {
    ...
    g_fail_next = false;
    buf_reset();                           /* fresh buffer */
    jcon_start(true, test_putc);           /* should clear sticky status */
    jcon_add_int("ok", 1);
    EXPECT_EQ(jcon_end(), JCON_OK);
    EXPECT_STR_EQ(buf_cstr(), "{\"ok\":1}");
}
...
static void test_float_basic(void) {
    ...
    EXPECT_EQ(jcon_end(), JCON_OK);
    EXPECT_STR_EQ(buf_cstr(), "{\"d\":3.14,\"f\":1.5}");
}
...
static void test_float_generic_dispatch(void) {
    ...
    jcon_add("f", f);
    EXPECT_EQ(jcon_end(), JCON_OK);
    EXPECT_STR_EQ(buf_cstr(), "{\"d\":2.5,\"f\":0.25}");
}
#endif
